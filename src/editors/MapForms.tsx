...
export function ItemForm({
  ...
  filePath: string;
  onFinish?: () => void;
  submit?: (update: TLocalItem, prevData: TLocalItem) => void;
}): JSX.Element {
  const [form] = Form.useForm();

  const [contentEditor, setContentEditor] = useState<string | TLocalContent>();
        ...
        switch (name) {
          ...
          case "itemForm": {
            if (submit) {
              ...
              connections.forEach((connection: TLocalConnection) => {
    ...
    >
      ...
      {contentEditor && (
        ...
        >
          <ContentEditor
            data={contentEditor}
            filePath={`${filePath}/${data.locationId}/${data.id}`}
...
export function LocationEditor({
  ...
  filePath: string;
  submit: (update: TLocalLocation, prevData: string | TLocalLocation) => void;
  deleteLocation?: (location: TLocalLocation) => void;
}): JSX.Element {
  const [form] = Form.useForm();
        ...
        >
          ...
          <Button
            ...
            onClick={() =>
              setItemForm({
                id: uuid(),
                locationId: typeof data === "string" ? data : data.id,
              })
            }
          >
            Add Item
